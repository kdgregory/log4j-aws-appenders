...
{
    // we create "helper" clients for all destinations, and separately configure their proxy
    // this one set is used for all tests, shut down at the end
...
//----------------------------------------------------------------------------
    ...
    {
        ec2Client = configureProxy(AmazonEC2ClientBuilder.standard()).build();
        ssmClient = configureProxy(AWSSimpleSystemsManagementClientBuilder.standard()).build();
    }
