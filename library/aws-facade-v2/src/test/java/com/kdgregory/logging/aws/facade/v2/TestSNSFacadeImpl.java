...
//----------------------------------------------------------------------------
    ...
    /**
     *  Constructs the exception that will be thrown when a request is throttled.
     *  This has been determined experimentally.
     */
    private SnsException throttlingException()
    {
        AwsErrorDetails details = AwsErrorDetails.builder().errorCode("Throttling").build();
        SnsException ex = (SnsException)SnsException.builder().message("Rate exceeded").awsErrorDetails(details).build();
        return ex;
    }
