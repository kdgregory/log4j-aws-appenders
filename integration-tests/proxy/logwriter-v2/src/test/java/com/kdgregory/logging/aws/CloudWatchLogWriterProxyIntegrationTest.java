...
{
    private final static String BASE_LOGGROUP_NAME = "CloudWatchLogWriterV2ProxyIntegrationTest";
...
//----------------------------------------------------------------------------
    ...
    {
        helperClient = configureProxy(CloudWatchLogsClient.builder()).build();
        iamClient = configureProxy(IamClient.builder().region(Region.AWS_GLOBAL)).build();
        stsClient = configureProxy(StsClient.builder()).build();
    }
...
//----------------------------------------------------------------------------
    ...
    {
        final int numMessages = 1001;
        final String roleName = "V2ProxyIntegrationTest-" + System.currentTimeMillis();
        ...
        try {
            ...
            config.setAssumedRole(role.arn());
            createWriter(config);
